...
Next, we define the basic.basicGUI.main method. This is the code that will run when the program starts 
...
them together. We then create and add everything to our basic.basicGUI.main window (which is a `Tabs` element):
```
```kotlin
...
```
```
We tell our basic.basicGUI.main window that we want it to be maximised and for the program to terminate if it is closed:
```
```kotlin
...
```
```
Finally, we make the basic.basicGUI.main window show itself:
```
```kotlin
    mainWindow.show()
